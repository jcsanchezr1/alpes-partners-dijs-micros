...
def init_db():
    ...
    from ...modulos.reportes.infraestructura.schema.reportes import Base
    
    logger.info("Modelos registrados:")
    logger.info(f"   - Reportes: {len([t for t in Base.metadata.tables.keys() if 'reporte' in t.lower()])} tablas")
